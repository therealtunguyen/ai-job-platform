...
export async function createNewJob(payload: CreateJobDTO): Promise<JobWithEmployer> {
...
export async function listJobs(limit = 50, offset = 0): Promise<JobWithEmployer[]> {
  const { data, error } = await supabase
    .from(TABLE)
    .select(`
      *,
...
export async function updateJob(id: string, updates: UpdateJobDTO, employerId: string): Promise<JobWithEmployer | null> {
  const { data, error } = await supabase
    .from(TABLE)
    .update({ ...updates })
    .eq("job_id", id)
