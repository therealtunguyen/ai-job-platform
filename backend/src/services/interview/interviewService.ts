    ...
    if (tags.length === 0 && jobDetails?.description) {
      ...
      if (tags.length === 0) {
        ...
        const words = descriptionText
          .split(/[,\s\n.;()]+/)
          .filter((word: string) => word.length > 4 && word.length < 20); // Filter for meaningful words
...
export const getUserInterviewsService = async (candidateId: string) => {
  try {
    ...
    if (error) {
      throw new Error(`Failed to retrieve user interviews: ${error.message}`);
    }
...
export const getInterviewByIdService = async (
  ...
  try {
    ...
    if (error) {
      throw new Error(`Failed to retrieve interview session: ${error.message}`);
    }
...
export const submitAnswerWithAiFeedback = async (
  ...
  try {
    ...
    if (feedbackError) {
      console.error("Error creating interview feedback record:", feedbackError);
      throw feedbackError;
    }
