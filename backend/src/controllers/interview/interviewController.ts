...
export const getInterviewById = async (req: Request, res: Response) => {
  try {
    ...
    const { data: conversationEntries, error: entriesError } = await supabase
    ...
    if (entriesError) {
      console.error("Error retrieving conversation entries:", entriesError);
  ...
  try {
    ...
    if (evaluationResult) {
      try {
        ...
        console.error("Error in atomic update operation:", atomicUpdateError);
