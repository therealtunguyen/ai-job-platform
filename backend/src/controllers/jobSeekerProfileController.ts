    ...
    async addLanguage(req: Request, res: Response) {
        try {
            ...
            if (!language_id) {
                return res
                    .status(400)
                    .json({ error: "language_id is required" });
            }

            // First check if the language exists
            const { data: languageExists, error: langError } =
                await this.supabase
                    .from("language")
                    .select("language_id")
            ...
            if (existing && existing.length > 0) {
                return res
                    .status(409)
                    .json({ error: "Language already added" });
            }
    ...
    async removeLanguage(req: Request, res: Response) {
        try {
            ...
            if (!languageId) {
                return res
                    .status(400)
                    .json({ error: "languageId is required" });
            }
    ...
    async addSocialNetwork(req: Request, res: Response) {
        try {
            ...
            if (!social_network_id) {
                return res
                    .status(400)
                    .json({ error: "social_network_id is required" });
            }

            // First check if the social network exists
            const { data: socialNetworkExists, error: snError } =
                await this.supabase
            ...
            if (snError || !socialNetworkExists) {
                return res
                    .status(404)
                    .json({ error: "Social network not found" });
            }
            ...
            if (existing && existing.length > 0) {
                return res
                    .status(409)
                    .json({ error: "Social network already added" });
            }
    ...
    async updateSocialNetwork(req: Request, res: Response) {
        try {
            ...
            if (!socialNetworkId) {
                return res
                    .status(400)
                    .json({ error: "socialNetworkId is required" });
            }
            ...
            const { data, error } = await this.supabase
                ...
                .update({
                    ...
                    profile_url:
                        profile_url !== undefined ? profile_url : undefined,
                })
                .eq("job_seeker_id", userId)
