...
async function insertJobSeekers(users: any[]) {
  const jobSeekers = users.filter((u) => u.user_metadata.type === "JOB_SEEKER");
  ...
  const entries = sessions.flatMap((session) => [
    {
      ...
      response_submitted_at: new Date(Date.now() + 300 * 1000).toISOString(), // 5 minutes later
    },
  ]);
  ...
  const experiences = jobSeekers.flatMap((seeker) => [
    {
      ...
      start_date: faker.date.past({ years: 5 }).toISOString().split("T")[0], // YYYY-MM-DD
  ...
  const certs = jobSeekers.flatMap((seeker) => [
    {
      ...
      issued_date: faker.date.past({ years: 2 }).toISOString().split("T")[0],
      expiry_date: faker.date.future({ years: 2 }).toISOString().split("T")[0],
    },
  ]);
