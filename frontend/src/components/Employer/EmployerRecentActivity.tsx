...
const EmployerRecentActivity = () => {
  ...
  useEffect(() => {
    const fetchEmployerApplications = async () => {
      ...
      try {
        setLoading(true);
        const apiUrl = API_PATHS.APPLICATIONS.GET_EMPLOYER_APPLICATIONS.replace(
          ":employerId",
          user.id.toString(),
        );
        ...
        console.log(
          "🎯 Recent applications with job seeker details:",
        ...
        <div className="rounded-xl border border-gray-100 bg-white shadow-lg">
          <div className="border-b border-gray-200 p-6">
            ...
            <p className="text-gray-600">
              Track recent applications to your job posts
            </p>
          </div>
          <div className="p-6">
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {applications.map((application) => {
              console.log(
                "🎨 Rendering application with job seeker data:",
                application,
              );
              return (
                <div key={application.application_id} className="group">
                  <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm transition-shadow hover:shadow-md">
                    <div className="mb-4 flex items-start justify-between">
                      ...
                      <span
                        className={`rounded-full px-2 py-1 text-xs font-medium ${getStatusColor(application.status || "pending")}`}
                      >
                    ...
                    <div className="flex items-center justify-between text-sm text-gray-500">
                      ...
                      <span className="font-medium">
                        {application.jobs?.min_salary &&
                        application.jobs?.max_salary
                          ? formatSalary(
                              application.jobs.min_salary,
                              application.jobs.max_salary,
                            )
          ...
          {applications.length === 0 && (
            <div className="py-12 text-center">
              ...
              <p className="text-gray-500">
                Applications to your job posts will appear here
              </p>
            </div>
          )}
        </div>
