...
const JobSeekerApplyRecently = () => {
  ...
  useEffect(() => {
    const fetchUserApplications = async () => {
      ...
      try {
        setLoading(true);
        const apiUrl = API_PATHS.APPLICATIONS.GET_USER_APPLICATIONS.replace(
          ":userId",
          user.id.toString(),
        );
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {applications.map((application) => {
              console.log(
                "🎨 Rendering application with job data:",
                application,
              );
              return (
                <div key={application.application_id} className="group">
                  <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm transition-shadow hover:shadow-md">
                    <div className="mb-4 flex items-start justify-between">
                      ...
                      <span
                        className={`rounded-full bg-green-300 px-2 py-1 text-xs font-medium text-green-800 ${getStatusColor(application.status || "pending")}`}
                      >
